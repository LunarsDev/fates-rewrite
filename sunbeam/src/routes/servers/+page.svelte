...
<section>
  <h1 class="best-server">Fates List</h1>
...
{#if !searchData}
  <Tag type={enums.TargetType.Server} tags={data.meta.tags} />
  ...
  <Section icon="fa-solid:certificate" title="Certified" id="certified-index">
    <CardContainer>
      {#each data.index.certified as server}
  ...
  <Section icon="fa-solid:sort-amount-up" title="Top Voted" id="top-voted-index">
    <CardContainer>
      {#each data.index.top_voted as server}
        <BotCard data={server} type={enums.TargetType.Server} rand={false} />
      {/each}
    </CardContainer>
  </Section>
